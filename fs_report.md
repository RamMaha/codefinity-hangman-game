...
def get_guessed_word(secret_word,guessed_letters):
    masked_string = "".join([letter if letter in guessed_letters  else HIDDEN_SYMBOL for letter in secret_word ])
...
def hints_match(pattern,candidate):
    if len(pattern) != len(candidate):
        return False
    matches = all(pattern[i] == "_" or pattern[i] == candidate[i] for i in range(len(pattern)))
    return matches

def show_possible_matches(pattern):
    matching_words = [word for word in WORDS if hints_match(pattern, word)]
